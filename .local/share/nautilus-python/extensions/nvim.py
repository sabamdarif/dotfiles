...
class OpenInGhosttyNvimAction(GObject.GObject, Nautilus.MenuProvider):
    def __init__(self):
        super().__init__()
    ...
    def _open_terminal_with_nvim(self, path):
        # Open Ghostty in fullscreen mode with nvim running in the specified directory
        # Using zsh with login shell to execute nvim
        cmd = ['ghostty', '--fullscreen', f'--working-directory={path}', '--gtk-single-instance=false', '-e', 'zsh', '-l', '-c', f'$(which nvim) "{path}"']
        ...
        if self._systemd:
            ...
            name = 'app-nautilus-com.mitchellh.ghostty-nvim-{}.scope'.format(pid)
    ...
    def _menu_item_activated(self, _menu, paths):
        for path in paths:
            self._open_terminal_with_nvim(path)

    def _make_item(self, name, paths):
        item = Nautilus.MenuItem(name=name, label='Open in Neovim (Ghostty)',
    ...
    def _paths_to_open(self, files):
        ...
        if 10 < len(paths):
            ...
            # to avoid accidentally spamming their desktop with dozens of
    ...
    def get_file_items(self, *args):
        ...
        if paths:
            return [self._make_item(name='GhosttyNvimNautilus::open_in_ghostty_nvim', paths=paths)]
        else:
            return []
    ...
    def get_background_items(self, *args):
        ...
        if paths:
            return [self._make_item(name='GhosttyNvimNautilus::open_folder_in_ghostty_nvim', paths=paths)]
        else:
            return []
